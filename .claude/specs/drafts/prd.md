...
- **Run-agent flow**: POST `/v1/agents/{id}/runs`. Orquestador reúne TODO el org+grupo context (sin RAG), hidrata Claude Code, envía prompt, retorna respuesta. Soporta **concurrencia de sesiones** (múltiples runs simultáneos de distintos usuarios, posiblemente sobre distintos agentes).
- **Concurrencia**: el sistema multi-tenant debe soportar sesiones concurrentes desde el diseño inicial. Cada run es independiente: workspace de Claude Code aislado por run, no compartido. El orquestador gestiona múltiples subprocesos simultáneos sin race conditions sobre el estado del agente.
- **El leak test pasa**: seed 2 orgs / 2 grupos / 3 usuarios con roles distintos; actuar como Usuario A; afirmar que cero filas y cero objetos de Org 2 o de Org 1/Grupo 2 son retornados; afirmar que operaciones fuera del rol retornan 403.
- **Demo vía curl y scripts**: no se requiere UI. La demo MVP se ejecuta con curl y scripts de test.
- Reuso de Houston: login de usuario (Supabase Auth + Google SSO PKCE), formato de agente (`houston.json` + `CLAUDE.md` + `.houston/`), helpers in-instance (`seed_agent`, `build_agent_context`), login relay de proveedor BYOA (invocado como subprocess desde Go).
...
- Rutas con shape compatible con Houston `/v1/*` para que agregar WebSocket sea aditivo post-MVP.
- Seed fixture reproducible (3 usuarios / 2 orgs / 2 grupos / 3 roles) para el leak test y los tests de RBAC.
- **Gestión de usuarios manual para MVP**: el `org:owner` crea, asigna roles y elimina usuarios directamente vía API/scripts. Sin email invite ni auto-provisioning por dominio.
- Manejo de errores claro para BYOA: si la cuenta de proveedor del org se revoca, el run falla con mensaje accionable.

### P2 — Nice to have (post-MVP)

- `context_shares` table para sharing controlado entre grupos/orgs.
- Event sourcing para historial de contexto inmutable (audit trail, rollback).
...
2. Orquestador persiste prompt (scoped), lanza Claude Code como subprocess en workspace aislado.
3. En paralelo: descarga TODOS los objetos del prefix org+grupo (sin RAG, con RLS).
4. Hidrata workspace, envía prompt, retorna `200 { result }`.
5. Concurrencia: otros runs de otros usuarios corren en subprocesos independientes sin interferencia.

**Demo MVP — curl y scripts:**
```bash
# Crear agente (autenticado como Usuario A)
curl -X POST /v1/agents -d '{"name":"my-agent","source":"blank"}'

# Correr agente
curl -X POST /v1/agents/{id}/runs -d '{"prompt":"Resume el contexto del grupo"}'

# Leak test
scripts/leak-test.sh  # retorna EXIT 0 si aislamiento íntegro, EXIT 1 si hay leak
```
...
|---|---|---|---|
| Supabase RLS + RBAC | Supabase | Bug en RLS o en las policies de rol que bypasea aislamiento o permisos | Leak test + RBAC test como acceptance gates; review de policies en cada PR |
| Claude Code CLI | Anthropic | Breaking changes en args o output format | Pin de versión; tests de integración contra CLI real |
| Houston `login_relay.rs` (Rust) | gethouston/houston | El relay está en Rust; el orquestador es Go | Invocar como subprocess (stdin/stdout piped) desde Go — el mismo patrón que Houston usa internamente. No reescribir en Go. |
| Google SSO / Supabase Auth | Google + Supabase | Cambios en PKCE flow o config de proveedor | Mismo Supabase project que Houston; compartir config de proveedor |
| Concurrencia de sesiones | Orquestador Go | Múltiples runs simultáneos sobre el mismo agente → race condition en archivos de estado | Workspace aislado por run (directorio temporal); el estado del agente en Supabase es la fuente de verdad, no el disco |

## §8. Rollout

- **Milestone 1 — Isolation cut (MVP, local):** modelo de datos con org+group+role; RLS en Postgres y Storage; RBAC básico (3 roles); flujos create-agent y run-agent con concurrencia; leak test + RBAC test pasan. Corre local: orquestador Go + Supabase + Claude Code. Demo vía curl y scripts.
- **Post-MVP — Operaciones y escala:** hosting cloud / sandboxes efímeros; RAG con pgvector; observabilidad por tenant; límites de uso por org; sharing controlado entre grupos/orgs; UI mínima.

## §9. Preguntas abiertas

- [x] ~~¿El `org:owner` puede ver el contenido de todos los grupos en tiempo real?~~ **Resuelto:** solo CRUD en MVP, sin observabilidad avanzada.
- [x] ~~¿Existe un rol super-admin de plataforma?~~ **Resuelto:** no existe.
- [x] ~~¿Cómo se invita a nuevos usuarios?~~ **Resuelto:** gestión manual para MVP (org:owner crea/asigna vía API/scripts).
- [x] ~~¿El `org:owner` puede leer conversaciones de otros grupos?~~ **Resuelto:** no puede leer conversaciones de otros grupos. Sí puede hacer todo lo demás (CRUD de grupos, usuarios y agentes en cualquier grupo de la org).
